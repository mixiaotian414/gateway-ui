...
class OrganizationCard extends Component{
  ...
  onLoadData = (treeNode) => {
    return new Promise((resolve) => {
      ...
      let orgId=treeNode.props.dataRef.value
      this.promise = request({
        url:"/gateway/organizationtree.json",
        method: 'post',
        data: {
  ...
  renderTreeNodes = (data) => {
    ...
    return data.map((item) => {
      ...
      return <TreeNode {...item} dataRef={item}  disableCheckbox={flag?false:true} />;
    });
  }
  emitEmpty = () => {
    //清空后获取焦点
...
// export default Form.create()(OrganizationCard)
export default connect(({ app }) => ({ app }))(Form.create()(OrganizationCard))
