...
import { Row, Col,Form,Button,Table,message,Modal,Card,Icon,Tree,Anchor,Input,Checkbox,Divider,Select,Radio } from 'antd'
import styles from './Business.less'

/**
 * @Title:业务模型编辑表属性模态框组件
 * @Description:子组件
 * @Author: chenshuai
 * @Time: 2018/7/10
 * @Version 1.0
...
let map = new Map()
let arraylist = []
class BusinessTableModal extends Component{
  ...
  render(){
    ...
    const {  LedgerType,changeBusinsessTableModal,changeColModal,businsessTableModal,colVisible,changeTableCode } = this.props;
    ...
    const tableModalProps = {
      visible: businsessTableModal,
      maskClosable: false,
      title: '物理表属性',
      wrapClassName: 'vertical-center-modal',
      width:"800px",
      onCancel:()=>{
        changeBusinsessTableModal(false)
        map.clear()
        /*this.setState({
          newattriList:[],
          attriList:[],
    ...
    const onSelectTree = (selectedKeys, info) => {
      ...
      if(this.props.businessTableCollist){
        if(this.props.businessTableCollist[0].tableName === info.node.props.title){
          this.setState({
            selectedTableKeys:selectedKeys,
            tablecode:this.props.businessTableCollist[0].tableCode,
            relacode:info.node.props.relaCode,
            //isselected:info.node.props.isSelected,
            attriList:arrayToTree(this.props.businessTableCollist[0].attriList.filter(_ => _.isNecessary !== '2'), 'attriCode', 'parentCode'),
            newattriList:this.props.businessTableCollist[0].attriList,
          })
        }
      }
      if(this.props.businessTableCollist){
        if(this.props.businessTableCollist[0].colList){
          this.props.businessTableCollist[0].colList.map((item)=>{
    ...
    const saveColumn = () =>{
      this.props.form.validateFields((err,fieldsValue)=>{
        if(err) return
        this.props.dispatch({
          type: LedgerType+'/modelcoladd',
          payload:{
            tableCode:this.props.TableCode,
            colCodes:selectedRowKeys,
          }
        }).then(()=>{
          this.props.dispatch({
            type: LedgerType+'/modeltabletree',
    ...
    const saveTableProps = () =>{
      ...
      this.props.dispatch({
        type: LedgerType+'/modelattrisave',
        payload:{
          relaCodes:formlist,
        }
      }).then(()=>{
    ...
    const name = this.state.newattriList&&this.state.newattriList.map((item,key) =>( item.attriCode ==="000011"?
        <Card id="000011" style={{width:'400px'}} >
    ...
    const queryType = this.state.newattriList&&this.state.newattriList.map((item,key) =>( item.attriCode ==="000007"?
        <Card id="000007" style={{width:'400px'}} key={key}>
          ...
          <Divider  className={styles.divider} />
          <Col span={20} offset={3}>
            <FormItem labelCol={{span:3}} wrapperCol={{span:25}}>
    ...
    const xlsType = this.state.newattriList&&this.state.newattriList.map((item,key) =>( item.attriCode ==="000008"?
        <Card id="000008" style={{width:'400px'}} key={key}>
          ...
          <Divider  className={styles.divider} />
          <Col span={20} offset={3}>
            <FormItem labelCol={{span:5}} wrapperCol={{span:20}} >
            ...
            <Col span={6}>
              ...
              <Card className={styles.body}>
                <div style={{overflow: 'auto', width:'100%', height: '280px'}}>
                  ...
                  >
                    {loop(this.props.businessTableCollist)}
                  </Tree>
                </div>
              </Card>
            </Col>
        ...
        >
          <Row>
            <Table
              ...
              size="small"
              rowKey={record => record.colCode}
              columns={columns}
              dataSource={this.props.linkColList}
              scroll={{ y: 300}}
...
export default Form.create()(BusinessTableModal)
