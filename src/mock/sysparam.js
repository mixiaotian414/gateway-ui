  ...
  'data|80-100': [
    {
      ...
      appId: /^[1234]\d{6}[1-9]$/,
      paraId: /^[1234]\d{6}[1-9]$/,
      'paraName|1': ["绩效考核系统","报表管理系统","客户关系系统","财务管理系统","总账管理系统"],
      paraCode: /^[1]\d{4}[1-9]$/,
      'appDesc|1': ["超级管理员","管理员","财务主任","业务经理","柜员"],
      'paraValue|1': ["类型1","类型2","类型3"],
      'openAppStatus|1':["否","是"],
      'isReadonly|1':["0","1"],
      lastUpdator: '@date',
      lastUpdateTime: '@date',
...
module.exports = {
  [`GET ${apiPrefix}/sysParam`](req, res) {
    const { query } = req
  ...
  [`GET ${apiPrefix}/sysParam/:paraId`](req, res) {
    const { paraId } = req.params
    console.log(paraId)
    const data = queryArray(database, paraId, 'paraId')
    if (data) {
      res.status(200).json({
