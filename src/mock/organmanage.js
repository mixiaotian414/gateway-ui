...
module.exports = {
  [`GET ${apiPrefix}/organmanage`](req, res) {
    const { query } = req
  ...
  [`GET ${apiPrefix}/organmanage/:pMenuId`](req, res) {
    const { pMenuId } = req.params
    const data = queryArray(database, pMenuId, 'pMenuId')
    if (data) {
      res.status(200).json({
          menuList2:data,
          tran_success:"1",
  ...
  [`GET ${apiPrefix}/organTreepersonnel`](req, res) {
    const { query } = req
  ...
  [`GET ${apiPrefix}/organTreepersonnelrole`](req, res) {
    const { query } = req
    let { pageSize, page } = query
    pageSize = pageSize || 10
    page = page || 1
