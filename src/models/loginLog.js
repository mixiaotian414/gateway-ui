...
import * as tableService from 'services/loginLog'
...
export default modelExtend(pageModel,{
  ...
  effects: {
    * query({payload={ page: 1, pageSize: 10 }}, {call, put,select}) {
      ...
      const { appId } = user
      payload = {...payload,appId:appId,logType:'01'}
      //事中列表data
    ...
    * delete ({ payload }, { select, call, put }) {
      ...
      if (response.success) {
        if (response.RSP_HEAD.TRAN_SUCCESS === '1') {
          //const {formValues} = yield (select(_ => _.loginLog))
